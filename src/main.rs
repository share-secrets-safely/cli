...
fn main() {
    ...
    let res = match matches.subcommand() {
        ("vault", Some(args)) => {
            let context = ok_or_exit(vault_context_from(args).context("vault context creation failed"));
            vault::do_it(&context)
        }
        ("extract", Some(args)) => {
            let context = ok_or_exit(extraction_context_from(args).context("extraction context creation failed"));
