...
(sandboxed
  ...
  (with "a vault with multiple secrets and multiple recipients"
    { import_user "$fixture/tester.sec.asc"
      "$exe" init --trust-model=web-of-trust --no-auto-import --gpg-keys-dir ./keys
      echo a | "$exe" add :a
      echo b | "$exe" add :subdir/b
    ...
    (when "the recipient to remove does not exist in the vault but in the keychain"
      ...
      it "fails" && {
        ...
        expect_run $WITH_FAILURE "$exe" recipient remove c@example.com
      }
    )
    ...
    (when "the recipient to remove does not exist in the vault and not in the keychain"
      it "fails" && {
        ...
        expect_run $WITH_FAILURE "$exe" recipient remove unkown@example.com
      }
    )
    ...
    (with "one recipient to remove which exists in the vault and another that does not"
      it "fails" && {
        ...
        expect_run $WITH_FAILURE "$exe" recipient remove b@example.com unkown@example.com
      }
