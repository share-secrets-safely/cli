...
(sandboxed
  (with "a single user"
    ...
    (with "an unnamed vault with an explicit secrets directory owned by 'tester'"
      "$exe" vault init --trust-model=web-of-trust --no-auto-import -k etc/keys --first-partition -r etc/recipients --secrets-dir secrets >/dev/null
...
(sandboxed
  (with "a single user"
    ...
    (with "an unnamed vault with an explicit secrets directory"
      in-space one/vault
      "$exe" vault init --trust-model=web-of-trust --no-auto-import -k etc/keys -r etc/recipients --secrets-dir secrets >/dev/null
