                ...
                .or_else(|(yaml_err, json_err)| {
                    yaml_rust::YamlLoader::load_from_str(&buf)
                        ...
                        .and_then(|v| match v.len() {
                            0 => return Err(err_msg("Deserialized a YAML without a single value").context("fatal")),
                            1 => return Err(err_msg("We expect single-document yaml files to be read by serde").context("fatal")),
                            _ => {
                                let mut m =
